    ...
    <transition name="fade-in" appear>
      <div class="auth-container">
        ...
        <div v-else class="form-wrapper">
          <h2 class="title">{{ showForgot ? 'Recuperar Contraseña' : 'Bienvenido de Nuevo' }}</h2>
          <p class="subtitle">{{ showForgot ? 'Ingresa tu correo para recibir instrucciones.' : 'Inicia sesión para continuar.' }}</p>

          <transition name="form-fade" mode="out-in">
            <form @submit.prevent="login" class="form" v-if="!showForgot">
              <div class="input-group">
  <label for="username">Nombre de usuario</label>
  <input id="username" v-model="username" type="text" placeholder="Ingresa tu usuario" required autocomplete="username" />
              ...
              <div class="input-group">
                ...
                <input id="password" v-model="password" :type="passwordFieldType" placeholder="********" required autocomplete="current-password" />
              ...
              <p class="toggle-link" @click="showForgot = true">¿Olvidaste tu contraseña?</p>
            </form>
            ...
            <form @submit.prevent="submitForgot" class="form" v-else>
              <div class="input-group">
                ...
                <input id="email" v-model="email" type="email" placeholder="tu@correo.com" required autocomplete="email" />
              </div>
              ...
              <p v-if="forgotMessage" class="feedback-msg success">{{ forgotMessage }}</p>
              <p v-if="error" class="feedback-msg error">{{ error }}</p>

              <p class="toggle-link" @click="showForgot = false">‹ Volver a Iniciar Sesión</p>
            </form>
          </transition>
        </div>
...
const submitForgot = async () => {
  ...
  try {
    ...
    forgotMessage.value = response.data.message || 'Si el correo existe, recibirás instrucciones.';
  } catch (err) {
    error.value = err.response?.data?.message || 'No se pudo procesar la solicitud.';
  } finally {
    loading.value = false;
  }
};
...
onBeforeMount(() => {
  const from = router.currentRoute.value?.query?.redirect || router.options.history.state.back;
