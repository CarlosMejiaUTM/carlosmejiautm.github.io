  ...
  <div class="detail-events-page">
    ...
    <section class="zona-detalle" v-else-if="zona">
          ...
          <template v-if="zona.imageUrls && zona.imageUrls.length > 0">
            ...
            <div v-if="zona.imageUrls.length > 1" class="thumbnails">
              <img
                ...
                :class="{
                  active: (selectedImage || zona.imageUrls[0]) === imageUrl,
                }"
      ...
      <div class="zona-reseñas">
        ...
        <p v-else class="no-reseñas">
          Sé el primero en dejar una reseña para este lugar.
        </p>
      </div>

      <div
        v-if="isAuthenticated && userRole === 'user'"
        class="reseña-form-wrapper"
      >
        <div class="reseña-form">
          ...
          <form @submit.prevent="submitReview">
            ...
            <div class="botones">
              <button
                v-if="!userAlreadyReviewed || isEditing"
                type="submit"
                class="btn-primary"
                :disabled="
                  isSubmittingReview ||
      ...
      <div v-if="!isAuthenticated" class="zona-cta">
        ...
        <div class="cta-buttons">
          <router-link to="/login" class="btn-accion"
...
<script setup>
  ...
  const getInitials = (name) =>
    name ? name.match(/\b\w/g)?.join('').slice(0, 2).toUpperCase() || 'A' : 'A';
  ...
  const submitReview = async () => {
    ...
    try {
      ...
      showNotification(
        err.response?.data?.message || 'Error al enviar la reseña.',
        'error'
      );
    } finally {
  ...
  const eliminarReseña = async () => {
    ...
    try {
      ...
      showNotification(
        err.response?.data?.message || 'Error al eliminar la reseña.',
        'error'
      );
    } finally {
