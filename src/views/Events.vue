<template>
  <section class="events-page">
    <div class="events-header">
      ...
      <p class="subtitle">Descubre las joyas turísticas que nuestro estado tiene para ofrecer</p>
    </div>
    ...
    <div v-else-if="paginatedZones.length > 0" class="zones-grid">
      <div v-for="zone in paginatedZones" :key="zone._id" class="zone-card">
        <router-link :to="{ name: 'DetailEvents', params: { id: zone._id } }" class="card-link-wrapper">
          <img v-if="zone.imageUrls && zone.imageUrls.length > 0" :src="zone.imageUrls[0]" :alt="`Imagen de ${zone.name}`" class="zone-image" />
          <div class="zone-info">
            <h3>{{ zone.name }}</h3>
            <p class="municipality"><i class="icon-pin"></i>{{ zone.municipality }}</p>
            <p class="rating">
        ...
        <div v-if="isAuthenticated && userRole === 'admin'" class="admin-buttons">
          <button class="btn-edit" @click.stop="openEdit(zone)">Editar</button>
          <button class="btn-delete" @click.stop="confirmDelete(zone)">Eliminar</button>
        </div>

        <button v-if="isAuthenticated && userRole === 'user'" class="favorite-btn" @click.stop="toggleFavorite(zone._id)" :disabled="togglingFavoriteId === zone._id" :aria-label="isFavorite(zone._id) ? 'Quitar de favoritos' : 'Agregar a favoritos'">
          <span v-if="togglingFavoriteId === zone._id" class="spinner small"></span>
          <i v-else class="icon-heart" :class="{ filled: isFavorite(zone._id) }"></i>
        </button>
      </div>
    </div>
    ...
    <nav v-if="!isLoading && totalPages > 1" class="pagination" role="navigation" aria-label="Paginación">
      <button :disabled="currentPage === 1" @click="changePage(currentPage - 1)">‹</button>
      <button v-for="page in totalPages" :key="page" :class="{ active: page === currentPage }" @click="changePage(page)">{{ page }}</button>
      <button :disabled="currentPage === totalPages" @click="changePage(currentPage + 1)">›</button>
    </nav>
    
    <EditZoneModal :is-open="editModalOpen" :zone-data="editZone" @close="closeEdit" @save="submitEdit" :is-saving="isSaving"/>
    <ConfirmDeleteModal :is-open="deleteConfirmOpen" @close="cancelDelete" @confirm="deleteZone" :is-deleting="isDeleting"/>
...
const loadInitialData = async () => {
  ...
  try {
    ...
    showNotification('Error al cargar la información. Revisa la consola y que el backend esté activo.', 'error');
  } finally {
