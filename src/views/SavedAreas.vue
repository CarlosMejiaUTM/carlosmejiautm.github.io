<template>
  <div class="saved-areas-page">
    <div class="container">
      ...
      <p class="subtitle">Los lugares que has guardado para tu próxima aventura.</p>
      ...
      <div v-else-if="favorites.length === 0" class="empty-state">
        ...
        <p>Parece que no has guardado ninguna zona. ¡Empieza a explorar para encontrar tus lugares preferidos!</p>
      ...
      <div v-else class="favorites-grid">
        <div v-for="zone in favorites" :key="zone._id" class="zone-card">
          <router-link :to="{ name: 'DetailEvents', params: { id: zone._id } }" class="card-link-wrapper">
            <img :src="zone.imageUrl" :alt="`Imagen de ${zone.name}`" class="zone-image" />
            <div class="zone-info">
              <h3>{{ zone.name }}</h3>
          ...
          <button class="btn-remove" @click.prevent="openConfirmDelete(zone)">
            <i class="icon-trash"></i> Quitar de favoritos
          </button>
        </div>
      </div>
    </div>
    
    <ConfirmDeleteModal :is-open="showConfirmModal" :is-deleting="isDeleting" @close="closeConfirmDelete" @confirm="handleRemoveFavorite" />
...
const fetchFavorites = async () => {
  ...
  try {
    ...
    showNotification('Error al cargar tus zonas favoritas. Intenta recargar la página.', 'error');
  } finally {
    loading.value = false;
  }
};
...
const handleRemoveFavorite = async () => {
  ...
  try {
    ...
    showNotification(`'${zoneToRemove.value.name}' eliminado de tus favoritos.`, 'success');
  } catch (err) {
    console.error('Error al eliminar favorito:', err);
    showNotification('No se pudo eliminar el favorito. Inténtalo de nuevo.', 'error');
  } finally {
